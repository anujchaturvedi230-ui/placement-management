...
const API =
  ...
  (import.meta.env.DEV
    ? "http://localhost:5000"
    : "https://placement-management-backend-7cqx.onrender.com");
...
function App() {
  ...
  async function changeRecruiterJobStatus(job, status) {
    try {
      const response = await fetch(`${API}/api/jobs/${job._id}/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getToken("recruiter")}`,
        },
        body: JSON.stringify({ status }),
      });
      const data = await response.json();
      if (!response.ok)
        return setGlobalMessage(data.message || "Job status update failed.");
      setGlobalMessage(data.message);
      await loadJobs();
    } catch (error) {
      console.error(error);
      setGlobalMessage("Job status update failed.");
    }
  }

  async function quickEditRecruiterJob(job) {
    const title = window.prompt("Job title", job.title || "");
    if (title === null) return;
    const description = window.prompt("Job description", job.description || "");
    if (description === null) return;
    const response = await fetch(`${API}/api/jobs/${job._id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${getToken("recruiter")}`,
    ...
    const data = await response.json();
    if (!response.ok)
      return setGlobalMessage(data.message || "Job update failed.");
    setGlobalMessage("Job updated successfully.");
    await loadJobs();
  }
  ...
  async function scheduleInterview(e) {
    ...
    try {
      ...
      // Keep the application status in sync with the scheduled interview.
  ...
  async function loadAdminData() {
    ...
    try {
      const [
        ...
        fetch(`${API}/api/admin/interviews`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ...
      if (interviewsResponse.ok)
        setAdminInterviews(interviewsData.interviews || []);
      if (companiesResponse.ok)
        setAdminCompanies(companiesData.companies || []);
  ...
  async function uploadResume(file) {
    ...
    if (!allowed.includes(file.type)) {
      setGlobalMessage("Please select a PDF, DOC, or DOCX resume.");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setGlobalMessage("Resume must be 5 MB or smaller.");
      return;
    }
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        const response = await fetch(`${API}/api/students/resume`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
        ...
        setGlobalMessage(data.message || "Resume uploaded successfully.");
      } catch (error) {
        setGlobalMessage(
          error.message || "Resume upload failed. Check backend connection.",
        );
      }
    };
    reader.onerror = () =>
      setGlobalMessage("Could not read the selected file.");
    reader.readAsDataURL(file);
  }

  async function downloadApplicantResume(studentId) {
    try {
      const response = await fetch(
        `${API}/api/recruiters/students/${studentId}/resume`,
        {
          headers: { Authorization: `Bearer ${getToken("recruiter")}` },
        },
      );
      const data = await response.json();
      if (!response.ok)
      ...
      link.href = data.resume.data;
      link.download = data.resume.fileName || "resume";
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (error) {
      setGlobalMessage(error.message || "Resume download failed.");
    }
  }

  async function downloadPlacementReport() {
    try {
      const response = await fetch(`${API}/api/admin/reports/placements.csv`, {
      ...
      setGlobalMessage("Placement report downloaded.");
    } catch (error) {
      setGlobalMessage(error.message || "Report download failed.");
    }
  }

  function openProfileEditor() {
    const current = student || recruiter;
    if (!current) return;
    setProfileDraft({
      name: current.name || "",
      email: current.email || "",
      ...
      batch: current.batch || "",
      cgpa: current.cgpa ?? "",
      skills: getArray(current.skills).join(", "),
  ...
  async function saveProfile(e) {
    e.preventDefault();
    clearMessage();
    const role = student ? "student" : "recruiter";
    const token = getToken(role);
    try {
      const response = await fetch(`${API}/api/${role}s/profile`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          ...profileDraft,
          skills: getArray(profileDraft.skills),
        }),
      });
      const data = await response.json();
      if (!response.ok) {
        setGlobalMessage(data.message || "Profile update failed.");
        return;
      }
      if (role === "student") {
        setStudent(data.student);
      ...
      setPage(role === "student" ? "student-dashboard" : "recruiter-dashboard");
    } catch (error) {
      console.error(error);
      setGlobalMessage("Profile update failed. Check backend connection.");
    }
  }
  ...
  function EditProfilePage() {
    ...
    const fields = isStudent
      ? [
          ...
          ["college", "College Name", "text"],
          ["branch", "Branch", "text"],
          ["batch", "Passing Year / Batch", "text"],
          ["cgpa", "CGPA", "number"],
          ["skills", "Skills (comma separated)", "text"],
        ]
      : [
          ["name", "Recruiter Name", "text"],
          ["email", "Email", "email"],
          ["phone", "Mobile Number", "tel"],
        ...
        <form className="form-card" onSubmit={saveProfile}>
          ...
          <div className="button-row">
            ...
            <button
              ...
              type="button"
              onClick={() =>
                setPage(isStudent ? "student-dashboard" : "recruiter-dashboard")
              }
            >
              Cancel
            </button>
    ...
    return (
      <div className="job-card">
        <div className="job-card-top">
          ...
          <span className="job-badge">{job.status || "Open"}</span>
        </div>
        ...
        {recruiterView && (
          <div className="button-row">
            ...
            <button
              type="button"
              className="secondary-btn"
              onClick={() => quickEditRecruiterJob(job)}
            >
              Edit Job
            </button>
            <button
    ...
    return (
      <div className="page-container">
        <div className="dashboard-header">
          ...
          <div className="button-row">
            <button className="secondary-btn" onClick={openProfileEditor}>
              Edit Profile
            </button>
        ...
        <div className="dashboard-card">
          <h3>My Resume</h3>
          <p className="muted">
            Upload a PDF, DOC, or DOCX file (maximum 5 MB).
          </p>
          {resumeName && (
            <p>
              <strong>Uploaded:</strong> {resumeName}
            </p>
          )}
          <label
            className="secondary-btn"
            ...
            {applicants.map((applicant) => (
              <div className="applicant-card" key={applicant.applicationId}>
                ...
                <div className="button-row wrap">
                  <button
                    className="secondary-btn"
                    onClick={() => downloadApplicantResume(applicant.studentId)}
                  >
                    Download Resume
                  </button>
                  <button
    ...
    return (
      <div className="page-container">
        <div className="dashboard-header">
          ...
          <div className="button-row">
            <button className="secondary-btn" onClick={downloadPlacementReport}>
                ...
                {adminInterviews.map((item) => (
                  <tr key={item._id}>
                    ...
                    <td>
                      <button
                        className="danger-btn small"
                        onClick={() => deleteAdminItem("interviews", item._id)}
                      >
                        Delete
                      </button>
                    </td>
  ...
  function renderPage() {
    switch (page) {
      ...
      case "edit-profile":
        return student || recruiter ? EditProfilePage() : LoginPage();
